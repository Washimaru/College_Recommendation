...
def test_R1_converged(profile, names):
    ranked = scored([("u1", 0.9), ("u2", 0.8), ("u3", 0.7)])
    resp = iter_loop(const_rank(ranked), MockLLM(top_k=2), profile, names,
...
def test_R2_confident(profile, names):
    ranked = scored([("u1", 0.9), ("u2", 0.8), ("u3", 0.7)])
    # keep_ids differ from top set so R1 cannot fire; confidence high -> R2.
    llm = StaticLLM(keep_ids=["u3"], confidence=0.95)
    resp = iter_loop(const_rank(ranked), llm, profile, names, max_iterations=5, top_k=2)
    assert resp.stop_reason == "R2_confident"


def test_R3_no_change(profile, names):
    ranked = scored([("u1", 0.9), ("u2", 0.8), ("u3", 0.7)])
...
def test_R4_iteration_cap(profile, names):
    ...
    resp = iter_loop(rank_fn, FlipLLM(), profile, names, max_iterations=4, top_k=2)
    assert resp.stop_reason == "R4_iteration_cap"
    assert len(resp.trace) == 4
    assert resp.trace[-1].iteration == 3


def test_precedence_R1_beats_R2(profile, names):
...
def test_run_loop_emits_iteration_then_final(profile, names):
    ranked = scored([("u1", 0.9), ("u2", 0.8)])
    events = list(run_loop(const_rank(ranked), MockLLM(top_k=2), profile, names,
                           max_iterations=5, top_k=2))
    assert events[-1]["type"] == "final"
    assert all(e["type"] == "iteration" for e in events[:-1])


def test_results_fall_back_to_top_k_when_keep_empty(profile, names):
    ranked = scored([("u1", 0.9), ("u2", 0.8), ("u3", 0.7)])
    llm = StaticLLM(keep_ids=[], confidence=0.2)
    resp = iter_loop(const_rank(ranked), llm, profile, names, max_iterations=2, top_k=2)
    assert [r.university_id for r in resp.results] == ["u1", "u2"]
